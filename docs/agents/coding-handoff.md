...
阶段 0 至阶段 8 已经完成并提交。当前准备实施阶段 9“HealthKit 与设置”。阶段 6 的真机后台、锁屏和真实长路线验收尚未完成，必须作为验证债务保留，但不阻塞后续模拟器开发。
...
实施阶段 9“HealthKit 与设置”：

- 增加 HealthKit capability 和按需授权状态展示。
- 通过 `HealthExporter` 单向写入已结束活动、暂停事件、可靠距离和可用路线。
- 使用产品全局活动 ID 保证写入幂等，并保存外部对象映射与静默重试状态。
- 保证拒绝授权或写入失败不影响本地记录、感受收束和作品更新。

完成标准以实施计划中的阶段 9 验收条件为准。HealthKit 写入与权限流程需要保留真机验收；不实现外部跑步导入、编辑、删除或超出首版范围的能力。
